...
class LedCycle extends GWModule {
  ...
  describe() {
    ...
    this.syncBlock(this.clk, Edge.Positive, [
      ...
      If (this.rst ['=='] (HIGH), this.onReset())
      .Else ([
        If (this.next ['=='] (HIGH), this.onNext())
        .Else ([
          If (this.prev ['=='] (HIGH), this.onPrev())
        ])
      ])
    ])
  }
...
const verilogCode = cg.toVerilog();

writeFile ('one-shot-debouncer.v', verilogCode, err => {
  if (err) {
    console.error(err);
  }
});
...
writeFile ('one-shot-debouncer-tb.v', tbCg.toVerilog(), err => {
  if (err) {
    console.error(err);
  }
});